<template>
  <div class="calendarView">
    <div class="no-print background">
      ...
      >
        <source :src="require('@/assets/media/octagon.mp4')" type="video/mp4" />
      </video>
      <video
        v-else
        class="video-background"
    ...
    <v-container class="mb-4 wrapper">
      <div class="text-lg-h2 text-md-h3 text-sm-h4 text-h5 handlee title mb-0">
        {{ state.title }}
      </div>
      ...
      <div
        ...
        v-if="state.ready && state.dates.length > 0"
        v-touch="{
          left: toNextDate,
      ...
      >
        ...
        >
          <v-card-actions class="no-print">
            ...
            <v-btn @click="toPreviousDate" icon="mdi-arrow-left" size="small" />
            <v-btn
              :disabled="!isDateValid(state.today.date())"
              @click="toDate(state.today.date())"
              class="handlee"
              variant="text"
            >
              Heute
          ...
          <v-row>
            ...
            >
              <v-card
                ...
                :variant="isPast(date) ? 'text' : 'outlined'"
...
const isToday = (date: moment.Moment) =>
  date.month() === state.today.month() && date.date() == state.today.date();
const isPast = (date: moment.Moment) => date < state.today;
...
const getQuery = () => {
  if (route.query.heute) {
    const date = state.year + "-12-" + route.query.heute;
